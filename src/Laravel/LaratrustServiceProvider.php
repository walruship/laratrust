...
namespace Walruship\Laratrust\Laravel;

use Walruship\Laratrust\Laratrust;
use Illuminate\Support\ServiceProvider;
use Walruship\Laratrust\Hashing\NativeHasher;
use Symfony\Component\HttpFoundation\Response;
use Walruship\Laratrust\Cookies\IlluminateCookie;
use Walruship\Laratrust\Sessions\IlluminateSession;
...
class LaratrustServiceProvider extends ServiceProvider
{
    ...
    protected function prepareResources()
    {
        ...
        $this->publishes([
            $config => config_path('walruship.laratrust.php'),
        ], 'config');
    ...
    protected function registerPersistences()
    {
        ...
        $this->app->singleton('laratrust.persistence', function ($app) {
            $config = $app['config']->get('walruship.laratrust.persistences');
    ...
    protected function registerSession()
    {
        $this->app->singleton('laratrust.session', function ($app) {
            return new IlluminateSession(
                $app['session.store'],
                $app['config']->get('walruship.laratrust.session')
            );
        });
    }
    ...
    protected function registerCookie()
    {
        $this->app->singleton('laratrust.cookie', function ($app) {
            return new IlluminateCookie(
                $app['request'],
                $app['cookie'],
                $app['config']->get('walruship.laratrust.cookie')
            );
        });
    }
    ...
    protected function registerUsers()
    {
        ...
        $this->app->singleton('laratrust.users', function ($app) {
            $config = $app['config']->get('walruship.laratrust.users');
    ...
    protected function registerRoles()
    {
        $this->app->singleton('laratrust.roles', function ($app) {
            $config = $app['config']->get('walruship.laratrust.roles');
    ...
    protected function registerCheckpoints()
    {
        ...
        $this->app->singleton('laratrust.checkpoints', function ($app) {
            $activeCheckpoints = $app['config']->get('walruship.laratrust.checkpoints');
    ...
    protected function registerActivations()
    {
        $this->app->singleton('laratrust.activations', function ($app) {
            $config = $app['config']->get('walruship.laratrust.activations');
    ...
    protected function registerThrottling()
    {
        $this->app->singleton('laratrust.throttling', function ($app) {
            $model = $app['config']->get('walruship.laratrust.throttling.model');

            $throttling = $app['config']->get('walruship.laratrust.throttling');
    ...
    protected function registerReminders()
    {
        $this->app->singleton('laratrust.reminders', function ($app) {
            $config = $app['config']->get('walruship.laratrust.reminders');
    ...
    protected function registerLaratrust()
    {
        ...
        $this->app->alias('laratrust', 'Walruship\Laratrust\Laratrust');
    }
    ...
    {
        $config = $this->app['config']->get('walruship.laratrust');
